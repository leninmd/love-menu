...
export async function verifyEmail(email: string, code: string) {
  return request<{ token: string; user: { id: string; email: string } }>('/v1/auth/email/verify', {
    method: 'POST',
    body: { email, code },
  })
}
...
export async function webauthnRegisterStart(email: string, deviceName: string) {
  return request<{ options: unknown; userId: string }>('/v1/auth/webauthn/register/start', {
    method: 'POST',
...
export async function webauthnLoginStart(email: string) {
  return request<{ options: unknown; userId: string }>('/v1/auth/webauthn/login/start', {
    method: 'POST',
    body: { email },
  })
}
...
export async function fetchMenu(restaurantId: string, search = '') {
  ...
  return request<MenuResponse>(`/v1/restaurants/${restaurantId}/menu${query}`)
}
...
export async function updateCategory(
  ...
  return request<{ status: string }>(`/v1/restaurants/${restaurantId}/categories/${categoryId}`, {
    method: 'PUT',
    body: { name, sortOrder },
    token,
  })
...
export async function updateDish(
  ...
  return request<{ status: string }>(`/v1/restaurants/${restaurantId}/dishes/${dishId}`, {
    method: 'PUT',
    body: payload,
    token,
  })
...
export async function deleteDish(token: string, restaurantId: string, dishId: string) {
  return request<{ status: string }>(`/v1/restaurants/${restaurantId}/dishes/${dishId}`, {
    method: 'DELETE',
    body: {},
    token,
...
export async function createOrder(token: string, cartId: string, restaurantId: string) {
...
export async function submitReview(
  ...
  return request<{ status: string }>(`/v1/orders/${orderId}/items/${itemId}/review`, {
    method: 'POST',
    body: { content },
    token,
  })
...
export async function fetchOwnerOrders(token: string, restaurantId: string) {
  return request<OwnerOrdersResponse>(`/v1/orders/restaurant/${restaurantId}`, { token })
}
...
export async function acceptOwnerOrder(token: string, orderId: string, restaurantId: string) {
...
export async function completeOwnerOrder(token: string, orderId: string, restaurantId: string) {
...
export async function createRestaurant(token: string, name: string, intro?: string) {
...
export async function uploadImage(token: string, file: File) {
  ...
  const baseUrl =
    (import.meta.env.VITE_API_BASE as string)?.replace(/\/$/, '') || 'http://localhost:3000'
...
export async function fetchGuestLevels(restaurantId: string) {
  return request<{ levels: GuestLevel[] }>(`/v1/restaurants/${restaurantId}/guest-levels`)
}
...
export async function updateGuestLevels(
  ...
  return request<{ status: string }>(`/v1/restaurants/${restaurantId}/guest-levels`, {
    method: 'PUT',
    body: { levels },
    token,
  })
