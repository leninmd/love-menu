...
- 风险认知：任何草率、缺乏推理依据或忽视约束的行为，都会导致整体系统失效与用户信任崩溃，必须以最高严谨度工作
...
5. 对照表格式
   - 用户要求你使用表格/对照表时，默认必须使用 ASCII 字符（文本表格）清晰渲染结构化信息
6. 尽可能并行执行独立的工具调用
...
气味警报：  
- 如果在解释「这里比较特殊所以……」超过两句，极大概率是设计问题，而不是实现问题
</philosophy_good_taste>
...
- **特别注意**：为防止记忆丢失，架构变更时也必须更新 `memory-bank/architecture.md`，详见「项目记忆库与计划管理」章节。
CLAUDE.md 内容要求：  
- 用最凝练的语言说明：  
  - 每个文件的用途与核心关注点  
  - 在整体架构中的位置与上下游依赖  
...
项目记忆库位于 `memory-bank/` 目录，是跨会话保持上下文的唯一持久化机制。你必须将其视为系统的「长期记忆」，并严格遵循以下规则进行读写。

### 记忆库文件职责

| 文件 | 职责 | 维护方式 |
|------|------|----------|
| `memory-bank/恋上菜单（Love Menu）项目综合文档.md` | 产品、架构与运维的**唯一事实来源**。所有设计决策、功能边界均以此为准 | 由用户手动更新，你只读不写 |
| `memory-bank/architecture.md` | 当前项目的文件结构树、模块职责、依赖关系的高层视图 | **由你全权负责生成和维护**，用户原则上不干预 |
| `memory-bank/progress.md` | 当前开发进度、已完成任务、下一步计划、阻塞问题 | 每次里程碑或阶段完成后由你主动更新 |
| `memory-bank/implementation-plan.md` | 与 `.opencode/plans/implementation-plan.md` 保持同步的副本 | 计划确定后双向同步，确保记忆库始终持有最新计划 |

### 初始化流程（首次会话或记忆库不完整时）

1. 检查 `memory-bank/architecture.md` 是否存在且非空。
   - 如果**为空或缺失**，你必须立即执行以下步骤：
     - 读取项目综合文档、现有文件结构、`.mcp.json` 和 `AGENTS.md`。
     - 基于技术方案章节（文档第5章），自动生成 `architecture.md`，内容包括：
       - 目标文件目录树（前端 `client/`、后端 `server/`、记忆库等）
       - 各目录和关键文件的职责说明
       - 模块间依赖关系概要
       - 技术栈清单
     - 将生成的内容写入 `memory-bank/architecture.md`，并向用户报告。
   - 如果**已存在且完整**，则直接加载并信任其中内容，除非与当前实际结构有显著冲突（此时应更新架构文档并说明差异）。
2. 读取所有其他记忆库文件，并在上下文中保持活跃。

### 变更管理规则

- **架构变更时**（创建/删除/移动文件、模块重组）：
  - 同步更新 `memory-bank/architecture.md` 中的目录树和职责说明，并简要记录变更原因。
  - 同时遵守 `<architecture_documentation>` 中关于 `CLAUDE.md` 的更新要求。
- **进度推进时**：
  - 每完成一个阶段（如「阶段1：项目基础结构搭建」）或重要里程碑，更新 `memory-bank/progress.md`：
    - 标记已完成任务为 `[x]`
    - 更新「当前状态」区块
    - 添加下一步计划
- **计划变更时**：
  - 在 Plan 模式下最终确定的实施计划，必须同步写入 `.opencode/plans/implementation-plan.md` 和 `memory-bank/implementation-plan.md`，保证两份文件内容一致。
  - 若在 Build 模式中发现计划需要调整，先切换到 Plan 模式讨论修改，确认后再次同步两份文件。

### 会话开始时的强制动作

每次新会话（重新启动 OpenCode 或 `/exit` 后再次进入）时，在接收任何开发指令之前，必须先执行：
/init
/add memory-bank/恋上菜单（Love Menu）项目综合文档.md
/add memory-bank/architecture.md
/add memory-bank/progress.md
/add memory-bank/implementation-plan.md

确保所有记忆库文件进入当前上下文，从而避免「系统失忆」。

### 同步检查

在对记忆库文件进行任何写入后，必须进行内部一致性检查：
- `architecture.md` 中的结构树是否与实际项目根目录的文件列表一致？
- `progress.md` 标记的「当前阶段」是否与 `implementation-plan.md` 中正在执行的任务对应？
- 若有矛盾，立即在回答中指出并修正，或向用户请求裁决。

### 严禁行为

- 禁止在未读取项目综合文档和当前文件结构的情况下凭空生成架构文档。
- 禁止修改 `memory-bank/恋上菜单（Love Menu）项目综合文档.md`（除非用户明确要求更新产品需求）。
- 禁止让 `memory-bank/architecture.md` 长期处于空白或过时状态——那是项目失忆的直接表现。
</memory_bank_management>
