...
const { registerStart, registerFinish, loginStart, loginFinish } = require('./webauthn')
...
app.post('/v1/auth/password/register', authLimiter, async (req, res) => {
  const email = String(req.body.email || '').trim()
  ...
  try {
    ...
    await setUserPassword(db, user.id, hash)
    const session = createEmailSession(user)
    return res.json({ token: session.token, user: session.user })
  } catch (error) {
    return res.status(error.status || 500).json({ error: error.message })
...
app.post('/v1/auth/password/login', authLimiter, async (req, res) => {
  const email = String(req.body.email || '').trim()
  const password = String(req.body.password || '')
  if (!email || !password) {
  ...
  try {
    ...
    const session = createEmailSession({ id: user.id, email: user.email })
    return res.json({ token: session.token, user: session.user })
  } catch (error) {
    return res.status(error.status || 500).json({ error: error.message })
  }
...
app.delete('/v1/users/me', authRequired, async (req, res) => {
  ...
  await db.run('DELETE FROM cart_items WHERE cart_id IN (SELECT id FROM carts WHERE user_id = ?)', [
    userId,
  ])
...
app.delete('/v1/restaurants/:id/categories/:categoryId', authRequired, async (req, res) => {
  ...
  try {
    ...
    await db.run('DELETE FROM categories WHERE id = ? AND restaurant_id = ?', [
      req.params.categoryId,
      req.params.id,
    ])
    return res.json({ status: 'ok' })
...
app.put('/v1/restaurants/:id/dishes/:dishId', authRequired, async (req, res) => {
  ...
  const sources = req.body.sources ? String(req.body.sources) : null
  const imageUrl =
    req.body.imageUrl !== undefined ? String(req.body.imageUrl || '').trim() || null : undefined
  if (!name) {
    return res.status(400).json({ error: 'name_required' })
  }
  try {
...
app.delete('/v1/restaurants/:id/dishes/:dishId', authRequired, async (req, res) => {
  ...
  try {
    await ensureRestaurantOwner(db, req.params.id, req.user.id)
    await db.run('UPDATE dishes SET is_deleted = 1 WHERE id = ? AND restaurant_id = ?', [
      req.params.dishId,
      req.params.id,
    ])
    return res.json({ status: 'ok' })
...
app.post('/v1/orders', authRequired, async (req, res) => {
  ...
  try {
    const orderId = await createOrderFromCart(db, cartId, req.user.id, restaurantId)
...
app.get('/v1/orders/:id/items', authRequired, async (req, res) => {
  const { db } = await dbPromise
  const order = await db.get('SELECT id, customer_id, status FROM orders WHERE id = ?', [
    req.params.id,
  ])
...
async function sendPushForOrder(db, order, title) {
  ...
  const subscriptions = await db.all('SELECT payload FROM push_subscriptions WHERE user_id = ?', [
    order.customer_id,
  ])
